...
\begin{itemize}
	...
	\item Sect.~1.3 (Methontology) has been moved to the end of the document (Sect.~\ref{sub:methontology_a_methodology_for_ontology_development}), but otherwise did not change.
	\item The section on design principles (Sect.~\ref{sub:general_design_decisions}) has been extended to reflect research done by the authors within the past year.
...
\subsection{Usability as a General Design Principles} % (fold)
\label{sub:general_design_decisions}

Apart from ``hard'' metrics for the evaluation of ontologies, so-called ``soft'' metrics for ontologies are relevant for facilitating better understanding by users and implementers, and therefore ultimately greater uptake.
