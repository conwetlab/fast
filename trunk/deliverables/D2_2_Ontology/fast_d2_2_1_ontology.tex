...
\newcommand\deliverableNumber{D2.2.1}
...
1.0 & 27.02.2009 & Knud M\"oller (NUIG) & final version ready for external review \\ \hline
\end{tabular}
\end{small}
...
\subsection{General Design Decisions} % (fold)
...
All of these references are discussed in \cite{urmetzer2009fast_state_of_the_art}, and we refer the reader to this deliverable for more detail.
...
\textcolor{white}{\textbf{Sources of Knowledge}} & FAST Architecture deliverable~\cite{solero2009fast_architecture}, FAST Requirements Specification~\cite{villoslada2009fast_requirements}, FAST State-of-the-art Document~\cite{urmetzer2009fast_state_of_the_art}, EzWeb documentation~\cite{lizcano2008ezweb} \\ \hline
\end{longtable}
\end{small}
\doublespacing
...
\texttt{hasInterests} & FOAF & \texttt{interest} & --- \\ \hline
\texttt{hasUserName} & FOAF & \texttt{accountName} & --- \\ \hline
\end{longtable}
\end{small}
\doublespacing
...
The facts which define the pre- and post-conditions of screens and screenflows in FAST will be modelled as individual RDF triples. In effect, this means that conditions, which are sets of facts, can be modelled as graph patterns using SPARQL notation (see \cite{sparql2008spec}). E.g., a simple pre-condition such as \emph{``There has to be a user''} could be expressed as in example~\ref{ex:simple_precondition}. Literally, this very simple pattern means \emph{``a variable ?user is of type sioc:User''}. In logical terms, it means something along the lines of \emph{``there exists a sioc:User''}. For the pre-condition to be fulfilled, it needs to be executed against the RDF graph in question (most likely the facts on the current canvas).
...
\clearpage
\section*{Appendix A (Ontology Code)}
\addcontentsline{toc}{section}{Appendix A (Ontology Code)}
...
\singlespacing
\begin{small}
\verbatiminput{ontology/fast_gadget.owl}
\end{small}

\clearpage
\doublespacing
\section*{Appendix B (Lists of Tables and Figures)}
\addcontentsline{toc}{section}{Appendix B (Lists of Tables and Figures)}
